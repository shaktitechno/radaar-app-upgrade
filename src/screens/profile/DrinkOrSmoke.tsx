...
const DrinkOrSmoke = (props: any) => {
    ...
    useEffect(() => {
        let data;
        data = JSON.stringify(profileData?.filter(item => item?.questions_name === 'drinkingSmokingFrequency'));
        ...
        options?.map(item => {
            data[0].options = [
                ...
                {
                    selected:
                        item === userDetails['drinkingSmokingFrequency']
                            ? true
                            : false,
                    text: item,
                },
